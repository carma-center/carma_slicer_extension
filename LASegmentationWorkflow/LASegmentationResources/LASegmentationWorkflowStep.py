...
class LASegmentationWorkflowStep( ctk.ctkWorkflowWidgetStep ) :
  ...
  def onEntry( self, comingFrom, transitionType ):
    ...
    super( LASegmentationWorkflowStep, self ).onEntry( comingFrom, transitionType )

  def onExit( self, goingTo, transitionType ):
    goingToId = "None"
    if goingTo: goingToId = goingTo.id()
    #print "-> onExit - current [%s] - goingTo [%s]" % ( self.id(), goingToId )
    super( LASegmentationWorkflowStep, self ).onExit( goingTo, transitionType )
  ...
  def validationSucceeded( self, desiredBranchId ):
    '''
    '''
    super( LASegmentationWorkflowStep, self ).validate( True, desiredBranchId )
  ...
  def validationFailed( self, desiredBranchId, messageTitle='Error', messageText='There was an unknown error. See the console output for more details!' ):
    ...
    super( LASegmentationWorkflowStep, self ).validate( False, desiredBranchId )
